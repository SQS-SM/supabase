...
const GET_CLIENT_QUOTE_FUNCTION_NAME: string = "get_client_quotes";
...
Deno.serve(async (request) => {
  try {
		...
		const { data, error } = await supabase(request.headers.get(AUTHORIZATION_HEADER)).rpc(GET_CLIENT_QUOTE_FUNCTION_NAME, {
